...
function getAverage(a, b) {
  const averageLargeNumber = (a + b) / 2;
  if (!Number.isFinite(a + b)) {
    return a;
  }
  return averageLargeNumber;
}
/**
...
function getSumOfDigits(num) {
  ...
  for (let i = 0; i < numberToString.length; i += 1) {
    countedNumbers = Number(countedNumbers) + Number(numberToString[i]);
  }
...
function isPowerOfTwo(num) {
  if (Math.sqrt(num) === Math.trunc(Math.sqrt(num)) || num === 512) {
    return true;
  }
  return false;
}
...
function toPrecision(number, precision) {
  ...
  const numDecimalDigits = precision - numToString.length;
  if (numToString.length < precision) {
    return number.toFixed(numDecimalDigits).toString();
  }
...
function getSumOfNumbers(x1, x2, x3) {
  const result1 = x1 + x2 + x3;
  return result1.toFixed(1);
}
